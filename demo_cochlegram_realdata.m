...
close all
fRange = [50, 8000];
%x1=detrend(audioread('T_1.wav'));
%x2=detrend(audioread('T_2.wav'));
...
x_1 = decimate(x1(1,:),1); % Down sampling 1
x_2 = decimate(x2(1,:),1); % Down sampling 2
x_ICA = [x_1; x_2];
%%
[x1_STFT,F_LOFAR_T,T_LOFAR_T] = stft(x_1,win,overlap,nfft,dfs);
figure,
set(gcf,'numbertitle','off','name', 'received result');
...
%%
[x2_STFT,F_LOFAR_T,T_LOFAR_T] = stft(x_2,win,overlap,nfft,dfs);
figure,
set(gcf,'numbertitle','off','name', 'received result');
surf(T_LOFAR_T,F_LOFAR_T,10*log10(abs(x2_STFT./max(abs(x2_STFT))))...
    ...
    set(gcf,'color','w')
        figure,
set(gcf,'numbertitle','off','name', '   mixed signal');
      plot(F_LOFAR_T,(db(abs(x2_STFT(:,1)./max(abs(x2_STFT(:,1)))))),'r-');
      %plot(F_LOFAR_T,abs(FD_sep_F{1,ii}(:,4)),'r-');
    %caxis([-20 0])
    xlim([0 2000])
    ylim([-30 0])
    xlabel('Frequency (Hz) - mixed signal','fontsize',12); ylabel('Amplitude','fontsize',12);
    set(gca,'fontsize',12)
    set(gcf,'color','w')
...
%% gamma tone
gf1 = gammatoneFast(x_1,128,fRange);%Construct the cochleagram use Gammatone filterbank
gf2 = gammatoneFast(x_2,128,fRange);%Construct the cochleagram use Gammatone filterbank

 
%%

cg = cochleagram(gf1);
[cg_STFT,F_LOFAR,T_LOFAR] = stft(cg(1,:),128,64,128,dfs);
figure,
set(gcf,'numbertitle','off','name', 'cochleagram result');
surf(T_LOFAR,F_LOFAR,10*log10(abs(cg_STFT./max(abs(cg_STFT))))...
       ,'edgecolor','none'); axis tight;
   shading interp;
   view(0,90);
    colorbar;
...
for k = 1:size(Rec,3)
    ...
    r(k,:) = synthesisFast(x1,mask,fRange); % logarit to linear
    r(k,:) = r(k,:)./max(abs(r(k,:)));
